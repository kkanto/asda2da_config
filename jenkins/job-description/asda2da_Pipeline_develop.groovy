...
def customLib = library identifier: 'asda2da_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asda2da_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asda2da.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asda2da", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asda2da_Pipeline_develop");

job.with {

    authenticationToken('CK6nwjIHyUlJvnku6nvbKikutS6jlDyB')
